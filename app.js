...
document.addEventListener('DOMContentLoaded', () => {
    fetch('memes.json')
        ...
        .catch(err => {
            ...
            showToast("Failed to load meme database. Please ensure you are running on a local web server (e.g. python -m http.server 8000).", "error");
        });
...
function renderGrid() {
    ...
    resultsCountLabel.textContent = `${filtered.length} meme${filtered.length !== 1 ? 's' : ''}`;
    ...
    filtered.forEach(item => {
        ...
        card.innerHTML = `
            ...
            <div class="relative w-full aspect-video bg-black overflow-hidden select-none cursor-pointer" onclick="openModal('${item.id}')">
                <!-- Neon Green Screen Overlay -->
                <div class="absolute inset-0 bg-emerald-500/10 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 border-b border-brand-500/25"></div>
                
                <!-- Video Play Indicator -->
                <div class="play-indicator absolute inset-0 flex items-center justify-center bg-slate-950/40 z-20 transition-all duration-300 group-hover:opacity-0 group-hover:scale-90">
                    <div class="h-12 w-12 rounded-full bg-slate-900/90 border border-slate-800/80 text-brand-400 flex items-center justify-center shadow-lg group-hover:bg-brand-500 group-hover:text-darkbg group-hover:border-transparent transition-all">
                        <i data-lucide="play" class="h-5 w-5 fill-current ml-0.5"></i>
                    </div>
                </div>

                <!-- Active Preview Dot -->
                <div class="absolute top-3 left-3 z-20 bg-emerald-500/90 text-darkbg font-display font-black text-[9px] px-2 py-0.5 rounded-full flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    <span class="h-1.5 w-1.5 rounded-full bg-darkbg animate-pulse"></span>
                    PREVIEW
                </div>

                <!-- Card Audio Toggle -->
                <button onclick="toggleCardMute(event, '${item.id}')" id="mute-btn-${item.id}" class="absolute top-3 right-3 z-30 p-1.5 bg-slate-950/70 border border-slate-850 hover:bg-slate-900 rounded-lg text-slate-400 hover:text-white transition-colors" title="Toggle audio">
                    <i data-lucide="volume-x" class="h-3.5 w-3.5"></i>
                </button>
            ...
            <div class="p-4 flex flex-col flex-1 justify-between">
                ...
                <div class="flex gap-2 mt-4 pt-3 border-t border-darkborder/30">
                    <a href="${item.path}" download="${item.name}.mp4" class="flex-1 flex items-center justify-center gap-1.5 py-2 bg-brand-500 hover:bg-brand-600 text-darkbg rounded-lg text-xs font-bold transition-all shadow-md" title="Download video file">
...
function toggleCardMute(event, itemId) {
    ...
    if (video.muted) {
        ...
        showToast("Audio Unmuted", "Hover will now play background sound.", "info");
    } else {
        video.muted = true;
